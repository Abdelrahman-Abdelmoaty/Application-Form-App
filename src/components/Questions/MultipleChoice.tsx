...
export default function MultipleChoice({ type }: { type: string }) {
  ...
  const handleSave = () => {
    const newQuestion = { id: ID, type: "MultipleChoice", value: question.current?.value, choices: choicesText, maxChoice: maxChoice.current?.value };
    dispatch(setQuestions({ type, newQuestion }));
  };
  const handleDelete = () => {
    setRemove(true);
    dispatch(deleteQuestion({ type, ID }));
  };
  return (
    <>
      {!remove && (
        <div className="flex flex-col mt-[10px]">
          ...
          <input placeholder="Type here" className="py-[15px] pr-[120px] mt-2 whitespace-break-spaces px-[10px] rounded-md border-[1px] border-gray-400 outline-none mb-[10px] font-semibold text-xl" ref={question} id={ID} />
          {Choices}
          <div className="flex justify-between items-center p-[20px]">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75" />
          ...
          <input placeholder="Type here" type="text" className="py-[15px] mb-2 flex-1 px-[10px] border-[1px] rounded-md border-gray-400 outline-none font-semibold text-xl" ref={maxChoice} />
