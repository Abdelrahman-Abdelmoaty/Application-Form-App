...
export default function Paragraph({ type }: { type: string }) {
  ...
  const handleSave = () => {
    const newQuestion = { id: ID, type: "Paragraph", value: question.current?.value };
    ...
    <>
      {!remove && (
        <div className="flex flex-col w-full my-[20px] relative">
          ...
          <textarea placeholder="Type here" rows={2} className="py-[5px] pr-[120px] whitespace-break-spaces px-[10px] border-b-[1px] border-gray-400 outline-none mb-[10px] font-semibold text-xl" ref={question} id={ID} />
